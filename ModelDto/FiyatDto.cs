using System;
using System.Collections.Generic;
using System.Linq;
using System.Threading.Tasks;

namespace UmotaCrmOkul.API.ModelDto
{
    public class FiyatDto
    {
        public double? Taksit0 { get; set; }
        public double? Taksit1 { get; set; }
        public double? Taksit2 { get; set; }
        public double? Taksit3 { get; set; }
        public double? Taksit4 { get; set; }
        public double? Taksit5 { get; set; }
        public double? Taksit6 { get; set; }
        public double? Taksit7 { get; set; }
        public double? Taksit8 { get; set; }
        public double? Taksit9 { get; set; }
        public double? Taksit10 { get; set; }
        public double? Toplam { get; set; }
        public DateTime? Tarih0 { get; set; }
        public DateTime? Tarih1 { get; set; }
        public DateTime? Tarih2 { get; set; }
        public DateTime? Tarih3 { get; set; }
        public DateTime? Tarih4 { get; set; }
        public DateTime? Tarih5 { get; set; }
        public DateTime? Tarih6 { get; set; }
        public DateTime? Tarih7 { get; set; }
        public DateTime? Tarih8 { get; set; }
        public DateTime? Tarih9 { get; set; }
        public DateTime? Tarih10 { get; set; }
        public List<OdemePlaniDto> OdemePlani { get; set; }
}
}
